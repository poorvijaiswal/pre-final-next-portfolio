...
const ResetPass = (props: ResetPassProps) => {
    ...
    const handleReset = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (success) {
            toast.success("Email sent Successfully", { position: 'top-center', autoClose: 4000, theme: 'colored' });
        }
    };
    useEffect(() => {
        if (error)
    ...
    return (
        ...
        >
            ...
            <button
                type='submit'
                className={`w-full text-white focus:ring-2 focus:ring-blue-500 font-medium rounded-lg text-sm px-5 py-2.5 text-center 
                bg-color-brand-orange hover:bg-color-brand-orange-s `}
            >
                Reset Password
            </button>
        </form>
